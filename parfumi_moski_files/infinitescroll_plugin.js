...
function initializeInfiniteScrollPlugin() {
	...
	$(function() {
		...
		//VEISS: SUPPORT POUR SIZEPRODUCTFILTER
		if($('#sizeproductfilter').length) {
			$(document).ajaxComplete(function(e, jqXHR, ajaxOptions){
				//If the request is one of the sizeproductfilter, then we need to reset the infinite scroll
				if(ajaxOptions.url.search('/modules/sizeproductfilter') != -1 
					&& ajaxOptions.url.search('&infinitescroll=1') == -1){
					
					kiwik.infinitescroll.log('AJAX call on sizeproductfilter');
					//la page peut être différente de 1 si on fait F5 par exemple
					var matches = /\/page\-([\d]*)/.exec(ajaxOptions.url)
					var page_called = 1;
					if (matches && matches.length) {
		...
		}//end if productsizefilter is loaded
		
		//VEISS: SUPPORT POUR CATEGORYPRODUCTFILTER
		if($('#categoryproductfilter-list').length) {
			$(document).ajaxComplete(function(e, jqXHR, ajaxOptions){
				//If the request is one of the categoryproductfilter, then we need to reset the infinite scroll
				if(ajaxOptions.url.search('/modules/categoryproductfilter') != -1 
					&& ajaxOptions.url.search('&infinitescroll=1') == -1){
					
					kiwik.infinitescroll.log('AJAX call on categoryproductfilter');
					//la page peut être différente de 1 si on fait F5 par exemple
					var matches = /\/page\-([\d]*)/.exec(ajaxOptions.url)
					var page_called = 1;
					if (matches && matches.length) {
		...
		}//end if categoryproductfilter is loaded
