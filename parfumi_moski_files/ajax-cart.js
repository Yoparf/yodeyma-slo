...
$(document).ready(function () {
	//Mutation observer de lo que falta para envio gratis para el cart overlay

	var fredevcontainer = document.querySelector(".menu-container .iqitfreedeliverycount");
	var observer_container = new MutationObserver(function (mutations) {
		if ($(".menu-container .iqitfreedeliverycount").hasClass("hidden")) {
			$("#cart-free-delivery-text").hide();
		} else {
			$("#cart-free-delivery-text").show();
		}
	});
	var config_container = { attributes: true, childList: true, characterData: true };
	//console.log(fredevcontainer);
	observer_container.observe(fredevcontainer, config_container);

	var fredevprice = document.querySelector(".menu-container .iqitfreedeliverycount .ifdc-remaining-price");
	var observer_price = new MutationObserver(function (mutations) {
		var price_text = $(".menu-container .iqitfreedeliverycount .ifdc-remaining-price").html();
		$("#cart-free-delivery").text(price_text);
	});
	var config_price = { attributes: true, childList: true, characterData: true };
	observer_price.observe(fredevprice, config_price);
	...
	// Veiss: Disable Shopping cart over
	var isOPC = $("body#order-opc").length;
	if (is_touch_device() && isOPC == 0) {
		...
		$("#header .shopping_cart a:first").hover(
			function () {
				if (ajaxCart.nb_total_products > 0 || cart_qty > 0) $("#header .cart_block").stop(true, true).fadeIn(450);
			},
			function () {
				setTimeout(function () {
					if (!shopping_cart.isHoveringOver() && !cart_block.isHoveringOver()) $("#header .cart_block").stop(true, true).fadeOut(450);
				}, 200);
			}
		);
	...
	$("#header .cart_block").hover(
		...
		function () {
			setTimeout(function () {
				if (!shopping_cart.isHoveringOver()) $("#header .cart_block").stop(true, true).fadeOut(450);
			}, 200);
		}
	);
...
var ajaxCart = {
	...
	overrideButtonsInThePage: function () {
		// Fix mas de una vez evento click
		...
		$(document).on("click", ".ajax_add_to_cart_button", function (e) {
			...
			if ($(this).prop("disabled") != "disabled") {
				...
				if ($(this).hasClass("ajax_add_to_cart_button_category")) {
					$quantity_field = $("#category_id_product_quantity_" + idProduct);
				} else {
					$quantity_field = $("#ajax_id_product_quantity_" + idProduct);
				}
				...
				ajaxCart.add(idProduct, null, false, this, quantity);
			}
		});
			...
			$(".cart_block_list:first dl.products dt").each(function () {
				...
				if (!stayInTheCart) {
					...
					if (removedProductId != null) {
						...
						$('dd[data-id="cart_block_combination_of_' + ids[0] + (ids[1] ? "_" + ids[1] : "") + (ids[2] ? "_" + ids[2] : "") + '"]').fadeTo(
							"fast",
							0,
							function () {
	...
	displayNewProducts: function (jsonData) {
		...
		$(jsonData.products).each(function () {
			...
			if (this.id != undefined) {
				...
				var domIdProduct =
					this.id + "_" + (this.idCombination ? this.idCombination : "0") + "_" + (this.idAddressDelivery ? this.idAddressDelivery : "0");
				...
				//else update the product's line
					...
					if (
						$.trim($('dt[data-id="cart_block_product_' + domIdProduct + '"] .quantity').html()) != jsonProduct.quantity ||
						...
						else $('dt[data-id="cart_block_product_' + domIdProduct + '"] .price').html(freeProductTranslation);
	...
	updateLayer: function (product) {
		if (ajax_popup) {
			var parameters = { id_product: product.id, token: static_token };
			$.ajax({
				type: "POST",
				headers: { "cache-control": "no-cache" },
				url: baseUri + "modules/blockcart/ajax_layer_features.php?rand=" + new Date().getTime(),
				dataType: "json",
				data: parameters,
				success: function (jsonData) {
					//Veiss: features tipo y volumen
					if (
						product.name == "CONTORNO ANTI-ARRUGAS" ||
						product.name == "SERUM REVITALIZANTE" ||
						product.name == "FLUIDO HIDRATANTE" ||
						product.name == "LECHE LIMPIADORA" ||
						product.name == "LOCIÓN TÓNICA" ||
						product.name == "GEL EXFOLIANTE" ||
						product.name == "CREMA DE MANOS"
					) {
						$("#layer_cart_product_feature_tipo").text("Para todo tipo de piel");
					} else if (product.name == "CREMA FACIAL ANTI-EDAD") {
						$("#layer_cart_product_feature_tipo").text("Para piel normal a seca");
					} else if (product.name == "CREMA VISO ANTI-ETÀ") {
						$("#layer_cart_product_feature_tipo").text("Per pelle normale e secca");
					} else if (
						product.name == "CONTORNO ANTI-RUGHE" ||
						product.name == "FLUIDO IDRATANTE" ||
						product.name == "SIERO RIVITALIZZANTE" ||
						product.name == "LATTE STRUCCANTE" ||
						product.name == "LOZIONE TONICA" ||
						product.name == "GEL EXFOLIANTE" ||
						product.name == "CREMA DE MANOS "
					) {
						$("#layer_cart_product_feature_tipo").text("Per qualsiasi tipo di pelle");
					} else if (
						product.name == "ANTI-WRINKLE EYE CONTOUR CREAM " ||
						product.name == "MOISTURIZING FLUID" ||
						product.name == "REVITALIZING SERUM" ||
						product.name == "CLEANSING MILK" ||
						product.name == "TONING LOTION" ||
						product.name == "GEL EXFOLIANTE" ||
						product.name == "CREMA DE MANOS "
					) {
						$("#layer_cart_product_feature_tipo").text("For all types of skin");
					} else {
						$("#layer_cart_product_feature_tipo").text(jsonData.tipo);
					}
					$("#layer_cart_product_feature_volumen").text(jsonData.volumen);
				},
			});
			...
			$(".layer_cart_img").html(
				'<img class="layer_cart_img img-responsive" src="' + product.image + '" alt="' + product.name + '" title="' + product.name + '" />'
			);
			...
			var remove_layer_cart = setTimeout(function () {
				ajaxCart.hideLayerCart();
			}, 4000);
			crossselling_serialScroll();
		} else $("body,html").animate({ scrollTop: 0 }, 400);
	},

	//@veiss: iban - quita el pop up de añadido correctamente al carrito
	hideLayerCart: function () {
		$(".layer_cart_overlay").hide();
		$("#layer_cart").fadeOut("fast");
	},
	...
	updateCartEverywhere: function (jsonData) {
		...
		if (parseFloat(jsonData.shippingCostFloat) > 0) $(".ajax_cart_shipping_cost").text(jsonData.shippingCost);
...
function crossselling_serialScroll() {
	...
	$(document).ready(function () {
		var flexmincross = 1;
		var flexmaxcross = 5;
		if ($("#layer_cart .crossseling-content").width() <= 280) {
			flexmincross = 1;
			flexmaxcross = 1;
		} else if ($("#layer_cart .crossseling-content").width() <= 440) {
			flexmincross = 2;
			flexmaxcross = 2;
		} else if ($("#layer_cart .crossseling-content").width() < 980) {
			flexmincross = 3;
			flexmaxcross = 3;
		} else if ($("#layer_cart .crossseling-content").width() >= 1240) {
			flexmincross = 5;
			flexmaxcross = 5;
		} else if ($("#layer_cart .crossseling-content").width() >= 980) {
			flexmincross = 4;
		...
		$(window).resize(function () {
			try {
				...
				if ($("#layer_cart .crossseling-content").width() <= 280) {
					$(selector).data("flexslider").setOpts({ minItems: 1, maxItems: 1 });
				} else if ($("#layer_cart .crossseling-content").width() <= 440) {
					$(selector).data("flexslider").setOpts({ minItems: grid_size_ms, maxItems: grid_size_ms });
				} else if ($("#layer_cart .crossseling-content").width() < 963) {
					$(selector).data("flexslider").setOpts({ minItems: grid_size_sm, maxItems: grid_size_sm });
				} else if ($("#layer_cart .crossseling-content").width() >= 1240) {
					$(selector).data("flexslider").setOpts({ minItems: grid_size_lg, maxItems: grid_size_lg });
				} else if ($("#layer_cart .crossseling-content").width() >= 963) {
					$(selector).data("flexslider").setOpts({ minItems: grid_size_md, maxItems: grid_size_md });
				}
			} catch (e) {
